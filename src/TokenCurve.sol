...
contract TokenCurve {
    ...
    function registerCurve(Curve calldata curve) external payable {
        ...
        if (ITokenMinter(curve.token).ownerOf(curve.id) != msg.sender)
            revert Unauthorized();
        if (curve.token == address(0) || curve.scale == 0)
            revert InvalidCurve();
        if (
            curve.burn_a > curve.mint_a ||
            curve.burn_b > curve.mint_b ||
            curve.burn_c > curve.mint_c
        ) {
    ...
    function support(
        ...
        if (floor > amountInCurrency) {
            if (curve.currency != address(0)) {
                ...
                if (
                    ICurrency(curve.currency).balanceOf(address(this)) >=
                    floor - amountInCurrency
                ) {
                    if (_price - amountInCurrency != msg.value)
                        revert InvalidAmount(); // Assumes 1:1 ratio between base coin and currency.

                    // Transfer currency.
                    ICurrency(curve.currency).transferFrom(
                        patron,
                        curve.owner,
                        amountInCurrency
                    );
                    ...
                    safeTransferETH(
                        ICurrency(curve.currency).owner(),
                        floor - amountInCurrency
                    );

                    // Record collections.
                    collected[_curveId].amountInCurrency += floor;
                    collected[_curveId].amountInStablecoin +=
                        _price -
                        burnPrice -
                        floor;
                    collected[_curveId].amountConverted +=
                        floor -
                        amountInCurrency;
                } else {
    ...
    function burn(
        ...
        // Validate mint conditions.
        Curve memory curve = curves[_curveId];
        if (ITokenMinter(curve.token).balanceOf(msg.sender, tokenId) == 0)
            revert Unauthorized();
    ...
    function getCollection(
        uint256 _curveId
    ) external view returns (Collected memory) {
    ...
    //     address owner;
    //     address currency = curves[_curveId].currency; // TODO: change to owner of currency not owner of curve
    //     uint256 amount = collected[_curveId].amountConverted;

    //     if (currency != address(0)) {
    //       owner = ICurrency(currency)
    //     }
